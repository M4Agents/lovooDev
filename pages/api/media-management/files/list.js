...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Validação rigorosa: não pode ser null, undefined, string vazia ou 'undefined' como string
    const hasValidFolderId = folder_id && folder_id !== '' && folder_id !== 'undefined' && folder_id !== 'null'
    
    if (hasValidFolderId) {
      console.log('📁 ANTES do filtro - folder_id recebido:', folder_id)
      console.log('📁 Tipo do folder_id:', typeof folder_id)
      query = query.eq('folder_id', folder_id)
      console.log('📁 DEPOIS do filtro aplicado')
    } else {
      console.log('⚠️ NENHUM folder_id válido fornecido:', folder_id)
      console.log('⚠️ Retornando TODOS os arquivos (SEM filtro)')
    }
    ...
    console.log('  - Total de arquivos retornados:', data?.length || 0)
    console.log('  - Count total:', count)
    if (data && data.length > 0) {
      console.log('  - Primeiros arquivos:', data.slice(0, 3).map(f => ({ 
    ...
    const files = (data || []).map(file => {
      ...
      return {
        ...
        folder_id: file.folder_id,  // ✅ USAR O VALOR REAL DO BANCO
        folder_path: null,
        uploaded_by: null,
        tags: [],
        is_favorite: false
