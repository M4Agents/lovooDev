// WEBHOOK UAZAPI V2 - CACHE BUSTING DEFINITIVO
// Novo arquivo para contornar cache persistente do Vercel
// Data: 2025-12-18 - SOLUÇÃO EMERGENCIAL

import { createClient } from '@supabase/supabase-js';

export default async function handler(req, res) {
  console.error('🚀 WEBHOOK V2 - CACHE BUSTING ATIVO - NOVO ARQUIVO');
  ...
  try {
    ...
    if (result.success) {
      console.log('✅ SUCESSO V2:', result.message_id);
      res.status(200).json({ 
        success: true, 
        message_id: result.message_id,
        message: 'WEBHOOK V2 - CACHE INVALIDADO!',
        timestamp: new Date().toISOString(),
        version: 'v2.0-emergency'
      });
    } else {
      console.log('⚠️ FILTRADO V2:', result.error);
...
async function processMessage(payload) {
  console.log('🔑 SUPABASE CONECTADO - WEBHOOK V2 COM RPC DIRETO');
  ...
  try {
    ...
    console.log('📞 DADOS EXTRAÍDOS V2:', {
      phoneNumber,
      senderName,
      content,
      messageType,
    ...
    if (instanceError || !instanceData || instanceData.length === 0) {
      console.error('❌ ERRO RPC INSTÂNCIA V2:', instanceError);
      return { success: false, error: 'Instância não encontrada: ' + instanceName };
    }
    
    const instanceInfo = instanceData[0];
    console.log('🏢 EMPRESA ENCONTRADA V2:', instanceInfo.company_name);
    ...
    if (processError) {
      console.error('❌ ERRO RPC PROCESS V2:', processError);
      return { success: false, error: 'Erro ao processar mensagem: ' + processError.message };
    }
    
    console.log('✅ SUCESSO RPC DIRETO V2:', result);
    return {
      success: true,
      message_id: result.message_id,
      contact_id: result.contact_id,
      conversation_id: result.conversation_id,
      message: 'Processado via RPC direto V2 (emergency fix)'
    };

  } catch (error) {
    console.error('❌ ERRO GERAL V2:', error);
