...
export const ReportFunnelSelector: React.FC<ReportFunnelSelectorProps> = ({
  ...
  const label =
    selected.length === 0
      ? t('funnel.allFunnels')
      : selected.length === 1
      ? (options.find((o) => o.id === selected[0])?.name ?? t('funnel.singleFallback'))
      : t('funnel.selectedCount', { count: selected.length })
      ...
      {open && (
        <div className="absolute top-full left-0 mt-1 w-64 bg-white border border-gray-200 rounded-lg shadow-lg z-50 py-1">
          ...
          {options.length === 0 && !loading && (
            <p className="px-3 py-2 text-xs text-gray-400">{t('funnel.noneFound')}</p>
          )}
        </div>
      )}
    </div>
