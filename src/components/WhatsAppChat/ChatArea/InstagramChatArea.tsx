...
export const InstagramChatArea: React.FC<InstagramChatAreaProps> = ({
  ...
  const handleSend = useCallback(async () => {
    ...
    setText('')
    await onSendMessage(trimmed, replyingTo?.ig_message_id ?? null)
  }, [text, sendLoading, connectionActive, onSendMessage, replyingTo])
              ...
              <div className="flex items-start gap-2 mb-2 px-3 py-2 bg-pink-50 border border-pink-200 rounded-lg">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-[10px] text-pink-600 font-medium mb-0.5">
                    {replyingTo.direction === 'inbound' ? 'Respondendo a mensagem recebida' : 'Respondendo sua mensagem'}
                  </p>
