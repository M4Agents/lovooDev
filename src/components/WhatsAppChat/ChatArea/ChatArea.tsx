...
export const ChatArea: React.FC<ChatAreaProps> = ({
    ...
    try {
      ...
      if (conv) {
        try {
          ...
          setContactPhotoUrl(resolvePhotoUrl(contactInfo?.profile_picture_url) || null)
        } catch (error) {
  ...
  const handleDragOver = (e: React.DragEvent) => {
    ...
    // Verificar se é mídia da biblioteca
    const hasMediaFile = e.dataTransfer.types.includes('mediafile')
    
    if (hasFiles || hasMediaFile) {
      setIsDragOver(true)
    }
  }
  ...
  const handleDrop = async (e: React.DragEvent) => {
    ...
    try {
      ...
      if (mediaFileData) {
        ...
        // Verificar se URL é endpoint local ou externa
        const isLocalEndpoint = mediaFile.preview_url.startsWith('/api/')
      ...
      // Arquivos do computador (comportamento original)
          ...
          {message.media_url && !isAudioMessage && actualMessageType !== 'image' && actualMessageType !== 'video' && (
            <div className="mb-1 flex items-center gap-1.5">
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="text-sm text-blue-600 underline truncate max-w-[220px]"
                title={t('message.actions.openFile')}
              >
                {(() => {
                  const seg = message.media_url.split('/').pop() ?? ''
                  // Remove timestamp prefix gerado no upload (ex.: "1778098002338_arquivo.pdf" → "arquivo.pdf")
                  const name = seg.replace(/^\d{10,}_/, '').replace(/_/g, ' ')
                  return name || t('message.actions.openFile')
                })()}
              </a>
            </div>
          )}
