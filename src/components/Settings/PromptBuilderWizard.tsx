...
function StepBasic({
  name, setName, model, setModel, language, setLanguage,
    ...
    <div className="flex flex-col items-center text-center py-8 gap-6">
      ...
      <div className="space-y-1.5">
        ...
        <p className="text-sm font-medium text-gray-600">
          <span className="text-gray-800">{agentName}</span> já está pronto para conversar
          com seus clientes 🎉
        </p>
...
export function PromptBuilderWizard({ companyId, onSaved, onAdvanced, onCancel }: Props) {
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      setPromptConfig(result.prompt_config)
      setStep(4)
    } catch (err: unknown) {
