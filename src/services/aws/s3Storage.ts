...
export class S3Storage {
  /**
   * Sanitize string to ASCII-safe format for S3 metadata
   * Removes accents, special characters, and non-ASCII characters
   */
  private static sanitizeForASCII(str: string): string {
    return str
      .normalize('NFD')                    // Decompor caracteres acentuados (á → a + ´)
      .replace(/[\u0300-\u036f]/g, '')    // Remover marcas diacríticas (´, `, ^, ~, etc)
      .replace(/[^\x00-\x7F]/g, '')       // Remover caracteres não-ASCII (emojis, símbolos)
  ...
  static async uploadToS3(options: UploadToS3Options): Promise<S3OperationResult<S3UploadResult>> {
    try {
      ...
      // Sanitize filename for metadata (correção caracteres não-ASCII)
      const sanitizedFilename = this.sanitizeForASCII(options.originalFileName);
      console.log('🔧 S3Storage - Filename sanitizado:', {
        original: options.originalFileName,
        sanitized: sanitizedFilename
      });

      // Prepare upload with progress tracking
      console.log('🔧 S3Storage - Preparando upload com rastreamento de progresso...');
      ...
      if (options.onProgress) {
        upload.on('httpUploadProgress', (progress) => {
          if (progress.loaded && progress.total) {
            ...
            console.log(`📊 S3Storage - Progresso: ${percentage}%`);
            options.onProgress(percentage);
          }
        });
      }

      console.log('✅ S3Storage - Upload preparado:', {
        bucket: credentials.bucket,
        key: s3Key,
        contentType: options.contentType,
