...
export const Settings: React.FC = () => {
  ...
  // Detectar retorno do OAuth Instagram (connected=1 ou ig_error=CODE)
  useEffect(() => {
    const integration = searchParams.get('integration');
    ...
    if (integration !== 'instagram') return;
    if (authLoading || isLoadingCompany) return;

    setActiveTab('integracoes');
    setIntegracoesTab('instagram');

    if (connected === '1') {
      const msg = account
        ? `Conta Instagram @${account} conectada com sucesso!`
        : 'Conta Instagram conectada com sucesso!';
      // Toast simples via alert nativo — substituir por toast do projeto se disponível
      ...
      {activeTab === 'integracoes' && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div
              onClick={() => setIntegracoesTab('instagram')}
              className="group cursor-pointer"
            >
              <div className={`bg-gradient-to-br from-purple-50 to-pink-100 border-2 rounded-lg p-4 transition-all duration-200 hover:shadow-md hover:scale-[1.02] ${
                integracoesTab === 'instagram'
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-1">
                      <h3 className="text-base font-semibold text-slate-900">{t('integrationsNav.instagram')}</h3>
                      <span className="px-2 py-0.5 bg-pink-100 text-pink-700 rounded text-xs font-medium">
                        {t('integrations.cards.instagram.badge')}
                      </span>
                    </div>
          ...
          {/* Sub-aba: Instagram */}
          {integracoesTab === 'instagram' && company?.id && (
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
              <InstagramConnectionPanel companyId={company.id} />
            </div>
          )}
