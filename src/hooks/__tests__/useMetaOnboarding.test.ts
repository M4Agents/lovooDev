...
afterEach(async () => {
  ...
  // React 18 passive effects flush: useEffect cleanups são agendados
  // assincronamente via MessageChannel. Sem este workaround, efeitos
  // de um teste vazam para o seguinte (ex: listeners de mensagem ativos,
  // retries de startOnboarding disparados por timers pendentes).
  // cleanup() remove componentes do tracking RTL; await act() drena a fila
  // do scheduler antes que o RTL padrão (LIFO) execute seu no-op.
  cleanup()
  await act(async () => {})
})
