...
async function processMessage(payload) {
  try {
    ...
    // Processar mídia se necessário - EXTRAIR URL APENAS
    if (isMediaMessage) {
    ...
    if (isMediaMessage && mediaUrl) {
      ...
      // DESCRIPTOGRAFIA VIA UAZAPI - CORREÇÃO CRÍTICA PARA PREVIEW
      if (message && message.id && mediaUrl.includes('whatsapp.net')) {
        console.log('🔓 DESCRIPTOGRAFANDO URL VIA UAZAPI:', message.id);
        ...
        try {
          ...
          if (uazapiResponse.ok) {
            ...
            if (uazapiData.fileURL) {
              console.log('✅ URL DESCRIPTOGRAFADA OBTIDA VIA UAZAPI');
              
              // PROCESSAR MÍDIA ROBUSTAMENTE - DOWNLOAD + UPLOAD PARA SUPABASE
              const processedUrl = await processMediaMessageRobust(message, supabase, uazapiData.fileURL, rawMediaType);
              finalMediaUrl = processedUrl;
              console.log('✅ MÍDIA PROCESSADA - URL FINAL:', finalMediaUrl.substring(0, 80) + '...');
            } else {
              console.log('⚠️ UAZAPI NÃO RETORNOU fileURL, usando URL original');
            }
          } else {
            console.log('⚠️ FALHA NA DESCRIPTOGRAFIA UAZAPI, usando URL original');
          }
        } catch (error) {
          console.error('❌ ERRO NA DESCRIPTOGRAFIA UAZAPI:', error);
        }
      } else {
        console.log('📝 URL NÃO É DO WHATSAPP OU SEM MESSAGE ID, usando diretamente');
      }
    }
