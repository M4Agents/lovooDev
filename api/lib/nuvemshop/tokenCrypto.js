// =============================================================================
// tokenCrypto — AES-256-GCM para tokens Nuvemshop
//
// Formato salvo: v1:<base64(IV[12] + authTag[16] + ciphertext)>
//
// Env var obrigatória:
//   NUVEMSHOP_TOKEN_ENC_KEY_V1 = 64 chars hex (32 bytes)
...
function getKeyV1() {
  const keyHex = process.env.NUVEMSHOP_TOKEN_ENC_KEY_V1;
  if (!keyHex) {
    throw new Error('[nuvemshop/tokenCrypto] NUVEMSHOP_TOKEN_ENC_KEY_V1 não configurada');
  }
  const key = Buffer.from(keyHex, 'hex');
  if (key.length !== 32) {
    throw new Error('[nuvemshop/tokenCrypto] Chave deve ter 64 chars hex (32 bytes / 256 bits)');
  }
  return key;
}

/**
 * Criptografa um access token Nuvemshop com AES-256-GCM.
 * @param {string} token Plaintext token (nunca logar este valor)
 * @returns {string} Formato: "v1:<base64(IV + authTag + ciphertext)>"
 */
export function encryptNuvemshopToken(token) {
  const key = getKeyV1();
...
/**
 * Descriptografa um access token Nuvemshop.
 ...
 */
export function decryptNuvemshopToken(encrypted) {
  if (typeof encrypted !== 'string') {
    throw new Error('[nuvemshop/tokenCrypto] Valor inválido para descriptografia');
  }

  const colonIdx = encrypted.indexOf(':');
  if (colonIdx < 0) {
    throw new Error('[nuvemshop/tokenCrypto] Formato de token desconhecido');
  }
  ...
  if (version === '1') {
    ...
    try {
      ...
      throw new Error('[nuvemshop/tokenCrypto] Falha na descriptografia do token');
    }
  ...
  throw new Error(`[nuvemshop/tokenCrypto] Versão de criptografia desconhecida: ${version}`);
}
