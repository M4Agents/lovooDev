// =============================================================================
// nuvemshopState — state JWT assinado para proteção CSRF no OAuth Nuvemshop
//
...
//
// Env var obrigatória:
//   NUVEMSHOP_STATE_SECRET = string aleatória forte
...
function getSecret() {
  const s = process.env.NUVEMSHOP_STATE_SECRET;
  if (!s) throw new Error('[nuvemshopState] NUVEMSHOP_STATE_SECRET não configurada');
  return s;
}
...
export function signState(payload, ttlMs = STATE_TTL_MS) {
  ...
  // O nonce (single-use) é gerado e armazenado no banco pelo caller (initiate.js).
  // Ele deve estar incluído em payload.nonce para ser embarcado no JWT.
