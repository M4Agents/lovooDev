...
export async function buildFunnelSnapshotMetrics(
  ...
  // Buscar valor real das oportunidades para calcular total_value por etapa.
  // Usa batching de 200 IDs por lote para evitar o limite de URL do PostgREST
  // em funis com muitas oportunidades (ex: > ~500 posições).
  ...
  for (let i = 0; i < oppIds.length; i += OPPS_BATCH_SIZE) {
    const batch      = oppIds.slice(i, i + OPPS_BATCH_SIZE)
    const batchIndex = Math.floor(i / OPPS_BATCH_SIZE)
    ...
    if (oppsErr) {
      throw new Error(
        `buildFunnelSnapshotMetrics/opps_values_failed batch=${batchIndex} size=${batch.length}: ${oppsErr.message}`,
      )
    }
