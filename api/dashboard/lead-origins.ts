// =====================================================
// GET /api/dashboard/lead-origins
//
// Volume, conversão e receita por canal de origem dos leads.
// Origem normalizada: LOWER+TRIM, NULL/vazio → 'desconhecida'.
//
// Query params:
...
//   user_id     (opcional)
//
// RBAC: mesmo padrão dos outros endpoints de dashboard.
// =====================================================
...
export default async function handler(req: any, res: any): Promise<void> {
  ...
  try {
    ...
    // 5. RPC get_dashboard_lead_origins
    const ctx = { companyId, period }

    const rawData = await withTiming(
      'dashboard.lead-origins',
      async () => {
        const { data, error } = await svc.rpc('get_dashboard_lead_origins', {
          p_company_id: companyId,
          p_start_date: resolvedRange.start,
          p_end_date:   resolvedRange.end,
          p_user_id:    effectiveUserId ?? null,
        })
        if (error) throw new Error(`get_dashboard_lead_origins: ${error.message}`)
        return (data ?? []) as any[]
      },
      ctx,
    )
    ...
    return res.status(200).json({
      ...
      meta: {
        ...
        total_origins: rawData.length,
      },
    })

  } catch (err: unknown) {
    logDashboardError('dashboard.lead-origins', err, {
      endpoint:  '/api/dashboard/lead-origins',
