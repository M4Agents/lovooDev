-- =====================================================
-- ROLLBACK: 20260625143000_add_sort_by_to_positions_rpc
-- Data: 25/06/2026
-- Objetivo: Reverter a migration que adicionou p_sort_by.
--
-- Origem do corpo: migration M6a aplicada e validada em produção
--   supabase/migrations/20260623450000_add_guards_kanban_rpcs.sql
--   (corpo exato das linhas 62-240)
--
-- Efeito: remove a versão de 13 parâmetros e restaura a de 12,
--         com todos os guards auth_user_can_access_funnel() preservados.
-- =====================================================

-- 1. Remover versão de 13 parâmetros introduzida pela migration
...
-- 2. Restaurar versão de 12 parâmetros (estado pós-M6a = estado atual em produção)
...
-- 3. Restaurar GRANT para a versão de 12 parâmetros
GRANT EXECUTE ON FUNCTION get_stage_positions_paged(
  UUID, UUID, UUID, TEXT, TEXT, INT, INT, INT, UUID[], TEXT, TIMESTAMPTZ, TIMESTAMPTZ
) TO authenticated;
