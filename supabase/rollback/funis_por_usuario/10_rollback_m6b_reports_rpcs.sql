-- =====================================================================
-- ROLLBACK M6b: Guards de Reports
-- Atualizado em: 2026-06-24 (corrigido para refletir estado real do banco)
--
-- O que este rollback faz:
--   1. Remove resolve_user_funnel_ids_access (criada por M6b)
--   2. Restaura get_funnel_overview ao estado pré-M6b
--      → LANGUAGE sql (sem BEGIN/END), sem CTE de guard
--   3. Restaura get_stage_time_metrics ao estado pré-M6b
--      → sem v_resolved_ids, usa p_funnel_ids diretamente
--   4. DROP + CREATE get_seller_performance (retorna user_id, usa auth.users)
--   5. DROP + CREATE get_cycle_time_metrics (retorna dimension/group_id/group_name)
--
-- Ordem de execução:
--   Este arquivo deve ser executado como parte do rollback APÓS M6b ser aplicada.
--   Se M6b não foi aplicada (falha), este arquivo NÃO deve ser executado.
--
-- ATENÇÃO: DEV e PRODUÇÃO compartilham o mesmo banco.
-- Rollback aqui afeta ambos os ambientes.
-- =====================================================================

-- ── 1. Remover helper criado por M6b ─────────────────────────────────
DROP FUNCTION IF EXISTS resolve_user_funnel_ids_access(UUID, UUID[]);


-- ── 2. Restaurar get_funnel_overview (LANGUAGE sql, sem guard) ────────
-- M6b converte esta função para LANGUAGE sql com CTE de guard.
-- Rollback restaura ao estado original: LANGUAGE sql, SELECT direto.
...
-- ── 3. Restaurar get_stage_time_metrics (sem v_resolved_ids) ─────────
-- M6b adiciona DECLARE v_resolved_ids e substitui p_funnel_ids.
-- Rollback restaura para usar p_funnel_ids diretamente.
...
-- ── 4. Restaurar get_seller_performance (user_id, auth.users, sem guard) ──
-- Retorna "user_id uuid" — nome de coluna real no banco.
-- M6b usou DROP + CREATE — rollback também usa DROP + CREATE.
...
-- ── 5. Restaurar get_cycle_time_metrics (dimension/group_id/group_name) ──
-- Retorna "dimension text, group_id uuid, group_name text" — nomes reais do banco.
-- M6b usou DROP + CREATE — rollback também usa DROP + CREATE.
