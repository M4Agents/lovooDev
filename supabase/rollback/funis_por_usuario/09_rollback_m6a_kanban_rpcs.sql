-- =====================================================================
-- ROLLBACK: M6a — Restaurar RPCs do Kanban ao estado pré-M6a
--
-- O que é revertido:
--   M6a adicionou guards auth_user_can_access_funnel() nas RPCs:
...
--     - move_opportunity
--
-- ⚠️  ATENÇÃO CRÍTICA — Diferenças identificadas entre M6a e estado original:
--
--   1. close_opportunity:
...
--      → Rollback restaura registros em opportunity_status_history (tabela de auditoria de status)
--
--   2. move_opportunity:
--      M6a usa INSERT INTO lead_stage_history (tabela diferente)
--      Original usa INSERT INTO opportunity_stage_history
--      M6a perdeu a validação: IF v_to_stage_type IS NULL THEN RAISE EXCEPTION
--      M6a perdeu a lógica 'active' que zera closed_at, actual_close_date e loss_reason
--
--   3. get_stage_positions_paged:
--      M6a remove tags aggregation, chat_contacts JOIN e LATERAL conv JOIN
--      M6a adiciona value_mode, computed_value, responsible_user_id na estrutura JSON
--      → Rollback restaura estrutura original com tags, profile_picture_url e chat_conversations
--
--   4. get_funnel_positions_with_photos:
--      M6a simplifica estrutura, remove reentry_count e masking de is_over_plan
--
-- Fonte: 02_current_state_kanban_rpcs.sql (estado capturado antes das migrations)
