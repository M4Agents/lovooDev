-- =============================================================================
-- FASE 5Z: process_webhook_message_safe — UPDATE PATH SYNC
-- =============================================================================
-- Arquivo: 20260612190001_process_webhook_update_path_sync.sql
-- Substitui: 20260611220000_fix_process_webhook_message_safe_assigned_to.sql
--
-- Mudança: UPDATE path passa a alinhar assigned_to ao responsável do lead.
--
-- Problema anterior:
--   UPDATE path não tocava assigned_to — divergências podiam persistir
--   indefinidamente em conversas existentes.
--
-- Solução:
--   Variável v_lead_resolved distingue dois estados de v_responsible_user_id = NULL:
--     FALSE → lead não encontrado → NÃO alterar assigned_to
--     TRUE  → lead encontrado     → alinhar ao CRM (pode resultar em NULL)
--
-- Regras:
--   Lead resolvido + responsável ativo   → assigned_to = responsible_user_id
--   Lead resolvido + responsável NULL    → assigned_to = NULL
--   Lead resolvido + responsável inativo → assigned_to = NULL
--   Lead não encontrado                  → assigned_to inalterado
--
-- Segurança:
--   • Nunca usa auth.uid() para assigned_to
--   • Nunca permite seller autoatribuir conversa
--   • RLS inalterada
-- =============================================================================
...
BEGIN
  ...
  -- Resolver responsável ativo. v_lead_resolved distingue "sem lead" de "lead sem responsável".
  IF v_lead_id IS NOT NULL THEN
    v_lead_resolved := true;  -- lead foi resolvido
    ...
    -- v_responsible_user_id = NULL quando: sem responsible_user_id ou responsável inativo
  END IF;
  ...
  IF v_conversation_id IS NULL THEN
    -- INSERT: preenche assigned_to desde 20260611220000 (inalterado)
  ...
  ELSE
    -- UPDATE: sincroniza assigned_to quando lead foi resolvido
    UPDATE chat_conversations
    SET
      contact_name    = COALESCE(NULLIF(p_sender_name, ''), contact_name),
      lead_id         = COALESCE(lead_id, v_lead_id),
      -- v_lead_resolved = false → ELSE: não altera (lead não identificado)
      -- v_lead_resolved = true, UUID  → THEN: alinha ao CRM
      -- v_lead_resolved = true, NULL  → THEN: limpa (responsável removido/inativo)
      assigned_to     = CASE
