-- =====================================================
-- MIGRATION: fix_owner_filter_use_responsible_user_id
-- Data: 25/06/2026
-- Objetivo: Corrigir o filtro p_owner_user_id nas RPCs canônicas
--           get_stage_positions_paged e get_funnel_stage_counts.
--
-- Diagnóstico (executado em 25/06/2026):
--   opportunities.owner_user_id está NULL em 3.473 de 3.475 registros.
--   O campo canônico usado pelo sistema para vincular responsável é
--   leads.responsible_user_id — o mesmo usado por auth_user_restricted_to_own_leads.
--
-- Correção (diff mínimo):
--   ANTES: AND (p_owner_user_id IS NULL OR o.owner_user_id      = p_owner_user_id)
--   DEPOIS: AND (p_owner_user_id IS NULL OR l.responsible_user_id = p_owner_user_id)
--
-- Estratégia: CREATE OR REPLACE FUNCTION (sem DROP)
--   A assinatura não muda (14 e 10 parâmetros permanecem idênticos).
--   Sem DROP → GRANTs existentes são preservados automaticamente.
--   Apenas o corpo interno é substituído.
--
-- Não alterado:
--   - Assinatura das RPCs
--   - DEFAULT NULL
--   - auth_user_restricted_to_own_leads
...
-- ══════════════════════════════════════════════════════════════════════════
-- 1. get_stage_positions_paged (14 parâmetros — assinatura inalterada)
-- ══════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  FROM (
    ...
    WHERE ofp.funnel_id  = p_funnel_id
      ...
      -- Filtro por responsável: corrigido para usar leads.responsible_user_id
      -- (campo canônico do sistema). NULL = sem filtro.
...
-- ══════════════════════════════════════════════════════════════════════════
-- 2. get_funnel_stage_counts (10 parâmetros — assinatura inalterada)
-- ══════════════════════════════════════════════════════════════════════════
