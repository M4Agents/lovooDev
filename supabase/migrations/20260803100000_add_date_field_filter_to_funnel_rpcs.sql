-- =====================================================
-- MIGRATION: add_date_field_filter_to_funnel_rpcs
-- Data: 03/08/2026
-- Objetivo: Adicionar p_date_field às RPCs do Kanban
--           para permitir filtrar por data de criação
--           (created_at) ou data de venda (closed_at).
--
-- Estratégia: DROP + CREATE nas 3 RPCs canônicas.
--   Compatibilidade retroativa garantida pelo DEFAULT
--   'created_at' — chamadas sem o parâmetro continuam
--   com o comportamento original.
--
-- Assinaturas:
--   get_stage_positions_paged     : 15 → 16 params
--   get_funnel_stage_counts       : 11 → 12 params
--   get_stage_opportunity_ids_filtered : 6 → 7 params
--
-- Valores válidos para p_date_field:
--   'created_at'  → opportunities.created_at (padrão)
--   'closed_at'   → opportunities.closed_at (data da venda)
-- =====================================================


-- ══════════════════════════════════════════════════════════════════════════
-- HELPER: resolve o valor correto do campo de data para o filtro
-- ══════════════════════════════════════════════════════════════════════════

-- Macro inline usada nas cláusulas WHERE das RPCs:
--   CASE WHEN p_date_field = 'closed_at' THEN o.closed_at ELSE o.created_at END


-- ══════════════════════════════════════════════════════════════════════════
-- 1. get_stage_positions_paged  15 params → 16 params
-- ══════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- ── Sanitização p_date_field: valor inválido → padrão ────
  ...
  FROM (
    ...
    WHERE ofp.funnel_id  = p_funnel_id
      ...
      -- Filtro de data: campo dinâmico (created_at ou closed_at)
      AND (
        CASE
...
GRANT EXECUTE ON FUNCTION get_stage_positions_paged(
  UUID, UUID, UUID, TEXT, TEXT, INT, INT, INT,
  UUID[], TEXT, TIMESTAMPTZ, TIMESTAMPTZ, TEXT, UUID, TEXT, TEXT
) TO authenticated;


-- ══════════════════════════════════════════════════════════════════════════
-- 2. get_funnel_stage_counts  11 params → 12 params
-- ══════════════════════════════════════════════════════════════════════════
...
GRANT EXECUTE ON FUNCTION get_funnel_stage_counts(
  UUID, UUID, TEXT, TEXT, INT, UUID[], TEXT, TIMESTAMPTZ, TIMESTAMPTZ, UUID, TEXT, TEXT
) TO authenticated;


-- ══════════════════════════════════════════════════════════════════════════
-- 3. get_stage_opportunity_ids_filtered  6 params → 7 params
-- ══════════════════════════════════════════════════════════════════════════

DROP FUNCTION IF EXISTS get_stage_opportunity_ids_filtered(
  UUID, UUID, UUID, TEXT, TEXT, INT
);
...
GRANT EXECUTE ON FUNCTION get_stage_opportunity_ids_filtered(
  UUID, UUID, UUID, TEXT, TEXT, INT, TEXT
) TO authenticated;
