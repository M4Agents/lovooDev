-- Migration: Aumentar timeout da RPC de 180s para 300s
-- Resolve erro: "canceling statement due to statement timeout"
-- Dá mais tempo para Uazapi responder ao gerar QR Code
    ...
    BEGIN
        ...
        SELECT * FROM http((
            ...
            ARRAY[
                ...
                http_header('User-Agent', 'LovoCRM-300sTimeout/1.0')
            ],
            'application/json',
        ...
        -- =====================================================
        -- ETAPA 2: OBTER QR CODE COM TIMEOUT DE 300 SEGUNDOS
        -- =====================================================
        v_step_start := clock_timestamp();
        
        -- ✅ CORREÇÃO: AUMENTAR TIMEOUT DE 180s PARA 300s (5 MINUTOS)
        PERFORM set_config('http.timeout_msec', '300000', true);
        
        -- Chamada com timeout aumentado
        SELECT * FROM http((
            'POST',
        ...
        v_debug_info := v_debug_info || jsonb_build_object(
            'step2_connect_300s', jsonb_build_object(
...
'Gera QR Code para conexão de instância WhatsApp com timeout de 300s (5 minutos). 
MODIFICADO: Timeout aumentado de 180s para 300s para resolver erro de statement timeout.
Ignora instâncias com deleted_at na validação de nome duplicado.';
