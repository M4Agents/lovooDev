-- =====================================================
-- MIGRATION: Adicionar filtro por intervalo de datas nas RPCs do Funil de Vendas
-- Data: 28/05/2026
-- Objetivo: Substituir p_period_days (últimos N dias) por
--           p_start_date / p_end_date (intervalos de data reais),
--           permitindo presets como Ontem, Mês passado, Trimestre e Custom.
--
-- RPCs alteradas:
...
--   p_period_days mantido com DEFAULT NULL como fallback.
--   Prioridade: p_start_date quando preenchido, senão p_period_days.
--   Sem qualquer parâmetro de data → sem filtro (retorna todos).
--
-- Segurança:
--   company_id, funnel_id, stage_id, RLS e permissões inalterados.
-- =====================================================


-- =====================================================
-- 1. get_stage_positions_paged
--    Adiciona: p_start_date TIMESTAMPTZ DEFAULT NULL
...
BEGIN
  ...
  FROM (
    ...
    WHERE ofp.funnel_id  = p_funnel_id
      ...
      -- Filtro de data: p_start_date tem prioridade; fallback para p_period_days
      AND (
        CASE
