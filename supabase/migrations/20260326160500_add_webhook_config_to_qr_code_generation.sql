-- Migration: Adicionar configuração de webhook na geração de QR Code
-- Resolve problema: Sistema não reconhece conexão após escanear QR Code
-- Webhook é necessário para Uazapi enviar eventos de conexão
...
BEGIN
    ...
    BEGIN
        ...
        -- =====================================================
        -- ETAPA 2: CONFIGURAR WEBHOOK (✅ NOVO)
        -- =====================================================
        v_step_start := clock_timestamp();
...
MODIFICADO: Adiciona configuração de webhook após criar instância para que sistema 
reconheça conexão quando usuário escaneia QR Code. Webhook envia eventos de 
connection.update e messages.upsert para Edge Function whatsapp-webhook.';
