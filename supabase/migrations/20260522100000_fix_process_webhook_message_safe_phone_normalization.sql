-- =====================================================================
-- Migration: Corrigir busca de lead por telefone em process_webhook_message_safe
-- Data: 2026-05-22
--
-- Problema:
--   A busca de lead usava igualdade exata: phone = p_phone_number
--   Leads criados via webhook externo armazenam o telefone com prefixo '+' (ex: +556184295688)
--   Mensagens WhatsApp chegam sem '+' (ex: 556184295688)
--   → busca falhava → conversa ficava sem lead_id → novo lead duplicado era criado
--
-- Correção:
--   Normalizar ambos os lados (remover não-dígitos) antes da comparação,
--   com fallback RIGHT(11) — mesmo padrão de create_lead_from_whatsapp_safe v5.
-- =====================================================================
...
BEGIN
  ...
  -- =====================================================
  -- 2. BUSCAR LEAD ASSOCIADO (com normalização de telefone)
  -- Normaliza ambos os lados para evitar mismatch entre
  -- formatos '+556184295688' (webhook) vs '556184295688' (WhatsApp).
  -- Fallback RIGHT(11) cobre variações DDI/DDD.
  -- =====================================================
...
'2026-05-22: Normalização de telefone na busca de lead (REGEXP_REPLACE + RIGHT(11)) para '
'evitar mismatch entre formatos com/sem + (webhook vs WhatsApp). '
'2026-05-06: Adicionado suporte a reply. '
