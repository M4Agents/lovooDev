-- =====================================================================
-- MIGRATION: Atualizar get_agent_report para usar completion_triggers
-- Data: 2026-08-07
--
-- Propósito:
--   Substituir o cálculo de completed_sessions de:
--     WHERE status = 'completed'        (nunca preenchido — sempre 0%)
--   por:
--     WHERE end_reason = ANY(assignment.completion_triggers)
--
--   Isso permite que cada empresa/assignment configure o que conta como
--   "atendimento concluído" para fins de relatório:
--     - IC Campo Limpo: completion_triggers = '{human_handoff}'
--     - Empresa sem configuração: completion_triggers = '{}' → rate = 0%
--
-- Dependências:
--   - 20260807110000_add_completion_triggers_to_assignments.sql
--   - 20260806100000_create_agent_report_rpc.sql
--
-- Mudanças na query:
--   1. period_sessions agora inclui end_reason (para comparação com triggers)
--   2. KPIs: completed_sessions usa LEFT JOIN com company_agent_assignments
--      e compara end_reason com completion_triggers do assignment
--   3. assignment_breakdown: adiciona completed_sessions e completion_rate por assignment
--   4. Sem alteração na assinatura da função — contrato externo preservado
-- =====================================================================
...
BEGIN
  ...
  -- ── 4. KPIs ──────────────────────────────────────────────────────────────────
  --
  -- period_sessions: base filtrada do período com end_reason incluído
  -- completed_sessions: sessões onde end_reason está na lista completion_triggers
  --   do assignment (LEFT JOIN 1:1 — sem risco de inflate)
  -- human_handoffs: COUNT DISTINCT de sessões com handoff ai_to_human no período
  ...
  SELECT
    COUNT(ps.session_id),
    -- Sessão concluída: end_reason deve estar na lista de triggers do assignment
  ...
  -- completed_sessions por assignment usa completion_triggers do próprio assignment.
  -- Créditos por assignment: agregados via ai_agent_execution_logs (sem inflate).
  ...
  FROM (
    ...
    assign_handoffs AS (
      -- DISTINCT session_id por assignment para evitar inflate
    ...
    assign_agg AS (
      -- Agrega sessões por assignment — sem JOIN com logs (evita inflate)
      SELECT
        a.assignment_id,
    ...
    -- Completed por assignment: usa completion_triggers do assignment
    assign_completed AS (
      SELECT
        a.assignment_id,
