-- =====================================================
-- MIGRATION: add_responsible_to_stage_positions_paged
-- Data: 09/09/2026
-- Objetivo: Incluir responsible_user_id (responsável pelo lead)
--           no JSON retornado pela RPC get_stage_positions_paged
--           (overload de 16 parâmetros — única usada pelo Kanban).
--
-- Causa raiz do bug: a RPC utilizava l.responsible_user_id nos filtros
-- (seller restriction e p_owner_user_id), mas não projetava o campo
-- no CTE base_page nem no jsonb_build_object do lead.
--
-- Mudanças funcionais (somente 2):
--   1. CTE base_page: l.responsible_user_id AS l_responsible_user_id
--   2. JSON lead:     'responsible_user_id', bp.l_responsible_user_id
--
-- Base: 20260909000000_optimize_get_stage_positions_paged_v2.sql
--
-- Preservado integralmente:
--   - Assinatura de 16 parâmetros com defaults
--   - SECURITY DEFINER, SET search_path = public
--   - Guard auth_user_can_access_funnel
--   - v_restricted (seller restriction por responsible_user_id)
--   - p_owner_user_id (filtro por responsável)
--   - Validações de p_tag_mode, p_sort_by, p_contact_attempts_state, p_date_field
--   - CTE base_page (estrutura, aliases, filtros, paginação, ordenação)
--   - Tags inline (subquery)
--   - chat_conversations (lateral join)
--   - opportunity_number, nuvemshop_order_id
--   - Ciclos de contato (contact_attempts_state e campos derivados)
--   - Todos os joins existentes
--
-- NÃO corrigido aqui (pré-existente, fora do escopo):
--   - Masking de is_over_plan/email/phone (não presente nesta overload)
--
-- Overloads legadas (8 e 10 parâmetros): NÃO alteradas.
-- =====================================================
...
BEGIN
  ...
  WITH base_page AS (
    SELECT
      ...
      l.responsible_user_id            AS l_responsible_user_id,  -- MUDANÇA 1: projetar responsible_user_id
      CASE
      ...
      jsonb_build_object(
        ...
        'opportunity', jsonb_build_object(
          ...
          'lead', jsonb_build_object(
            ...
            'responsible_user_id', bp.l_responsible_user_id  -- MUDANÇA 2: incluir no JSON do lead
          )
        )
      ) AS row_data,
      bp.sort_custom,
