-- =====================================================
-- MIGRATION: add_cycle_fields_to_get_stage_positions_paged
-- Data: 08/07/2026
-- Fase: 5 — CycleStatusBadge no Kanban
--
-- Objetivo:
--   Expor 7 campos de ciclo de contato no retorno da RPC
--   get_stage_positions_paged, permitindo que o LeadCard
--   exiba o CycleStatusBadge sem chamadas extras por card.
--
-- Campos adicionados ao jsonb_build_object raiz (nível ofp):
--   contact_attempts_state     — CHECK: 'none'|'cycle_open'|'waiting'|'eligible'
--   current_contact_cycle_id   — UUID FK para contact_attempt_cycles
--   contact_cycle_opened_at    — TIMESTAMPTZ de abertura do ciclo atual
--   total_contact_attempts     — INT total de tentativas na oportunidade
--   last_contact_attempt_at    — TIMESTAMPTZ da última tentativa
--   last_cycle_close_reason    — TEXT motivo do último fechamento de ciclo
--   eligible_for_new_cycle_at  — TIMESTAMPTZ de elegibilidade para novo ciclo
--
-- Todos os campos já existem em opportunity_funnel_positions (alias ofp).
-- Nenhum JOIN adicional foi introduzido.
--
-- Estratégia: CREATE OR REPLACE FUNCTION (sem DROP)
--   - Assinatura inalterada (14 parâmetros)
--   - Retorno continua JSONB
--   - SECURITY DEFINER preservado
--   - SET search_path preservado
--   - Guards preservados (auth_user_can_access_funnel, auth_user_restricted_to_own_leads)
--   - Filtros preservados (search, origin, dates, tags, owner)
--   - Ordenação preservada (p_sort_by + tiebreakers)
--   - JOINs preservados (opportunities, leads, chat_contacts, conversations)
--   - GRANTs preservados (CREATE OR REPLACE mantém permissões)
--   - Nenhuma alteração em get_funnel_stage_counts
--
-- Rollback: supabase/rollback/20260708140000_rollback_add_cycle_fields_to_get_stage_positions_paged.sql
-- =====================================================
